...
%
% Once loaded, the variables stored inside of the .mat file can be accessed
...
%clear all variables, close all windows, clear the command window
clear all
close all
clc

data = load('Intramural_Field_Noise_Profile.mat'); %load the signal profile
...
VariableNames = {
    'SpectrumCentroidFrequency';
    'SpectrumCentroidValue';
    'SpectrumFirstPeakFrequency';
    'SpectrumFirstPeakValue';
    'SpectrumSecondPeakFrequency';
    'SpectrumSecondPeakValue';
    'SpectrumThirdPeakFrequency';
    'SpectrumThirdPeakValue';
    'SpectrumFourthPeakFrequency';
    'SpectrumFourthPeakValue';
    'SpectrumFifthPeakFrequency';
    'SpectrumFifthPeakValue';
    'SilencePercentage';
    'ShortTimeZeroCrossingRate_STZCR';
    'AverageSTZCR';
    'ZeroCrossingRate';
    'ShortTimeEnergy_STE';
    'AverageSTE';
 ...
 T = table(...
    data.SpectrumCentroidFreq, data.SpectrumCentroidValue,...
    data.SpectrumPeakFreqs(:,1), data.SpectrumPeakValues(:,1),...
    data.SpectrumPeakFreqs(:,2), data.SpectrumPeakValues(:,2),...
    data.SpectrumPeakFreqs(:,3), data.SpectrumPeakValues(:,3),...
    data.SpectrumPeakFreqs(:,4), data.SpectrumPeakValues(:,4),...
    data.SpectrumPeakFreqs(:,5), data.SpectrumPeakValues(:,5),...
    data.SilencePercentage, data.STZCR(:,1), data.AverageZCR, data.ZCR,...
    data.STEnergy(:,1), data.AverageEnergy, data.Energy,...
    data.SpectrumFlux(:,1), 'VariableNames', VariableNames)

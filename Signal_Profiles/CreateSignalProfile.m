...
function ProfileFileName = CreateSignalProfile(waveform, Fs, SpectrumData)
numDataSets = size(waveform,1);
if(nargin < 3)
...
while(done==0)
    if(creationStep == 0)
        ...
        FeatureData = ProfileDetectionFeatures(waveform, SpectrumData, Fs);
        Name = profileName;
        IsDrone = isDrone;
